...
By default, the SDK validates requests against bundled JSON schemas before sending. To use a remotely-fetched schema (e.g., to get the latest target definitions without updating the SDK):

```go
schema, err := decodo.LoadRemoteSchema(decodo.RemoteSchemaOptions{
TTLMs: 3_600_000, // cache for 1 hour
})
if err != nil {
log.Fatal(err)
}
