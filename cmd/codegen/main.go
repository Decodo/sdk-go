...
func fetchIR() (*IR, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("listing IR: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&list); err != nil {
		return nil, fmt.Errorf("decoding GCS list: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("fetching IR: %w", err)
	}
...
func generateTargetsFile(ir *IR, outDir string) error {
	...
	for _, key := range targetKeys {
		...
		for _, propKey := range propKeys {
			...
			// Add enum comment if applicable
			if len(prop.Enum) > 0 {
	...
	// TargetMeta type
	sb.WriteString("// TargetMeta contains metadata about a scrape target.\n")
	sb.WriteString("type TargetMeta struct {\n")
	sb.WriteString("\tGroup          string\n")
	sb.WriteString("\tResponseFormat string\n")
	sb.WriteString("\tParameters     []string\n")
	sb.WriteString("}\n\n")

	// TargetMetaMap
	sb.WriteString("// TargetMetaMap contains metadata for all targets.\n")
	sb.WriteString("var TargetMetaMap = map[Target]TargetMeta{\n")
	for _, key := range targetKeys {
		target := api.Targets[key]
		constName := toTargetConstName(key)
...
func generateParametersFile(ir *IR, outDir string) error {
	...
	outPath := filepath.Join(outDir, "parameters.go")
	if err := os.WriteFile(outPath, []byte(sb.String()), 0644); err != nil {
		return fmt.Errorf("writing parameters.go: %w", err)
	}
	fmt.Printf("Generated %s (%d parameters)\n", outPath, len(paramOrder))
	return nil
}

func generateRequestSchemasFile(ir *IR, outDir string) error {
	api := ir.APIs.WebScrapingAPI
	...
	sb.WriteString("// Code generated by cmd/codegen; DO NOT EDIT.\n\n")
	sb.WriteString("package decodo\n\n")
	sb.WriteString("// RequestSchemas contains raw JSON schemas for validating scrape requests.\n")
	sb.WriteString("var RequestSchemas = map[Target]string{\n")
	...
	for _, key := range targetKeys {
		...
		var raw interface{}
		if err := json.Unmarshal(target.ParameterSchema, &raw); err != nil {
			continue
		}
		...
		schemaStr := string(compacted)
		sb.WriteString(fmt.Sprintf("\t%s: `%s`,\n", constName, schemaStr))
	}

	sb.WriteString("}\n")

	outPath := filepath.Join(outDir, "request_schemas.go")
	if err := os.WriteFile(outPath, []byte(sb.String()), 0644); err != nil {
		return fmt.Errorf("writing request_schemas.go: %w", err)
	}
...
func main() {
	...
	if err := generateTargetsFile(ir, outDir); err != nil {
	...
	if err := generateRequestSchemasFile(ir, outDir); err != nil {
		fmt.Fprintf(os.Stderr, "Error generating request schemas: %v\n", err)
		os.Exit(1)
	}
