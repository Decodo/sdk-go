...
func loadRemoteIR(opts RemoteSchemaOptions) (*remoteIR, error) {
	cachePath, err := expandPath(defaultIRCachePath)
	if err != nil {
		return nil, fmt.Errorf("resolving cache path: %w", err)
	}
	if opts.CachePath != "" {
		cachePath, err = expandPath(opts.CachePath)
		if err != nil {
			return nil, fmt.Errorf("resolving cache path: %w", err)
		}
	}
	...
	if cached != nil && cached.ResolvedURL == resolvedURL {
		...
		if cached.IR.Version == resolvedVersion {
			return &cached.IR, nil
		}
	}
...
func writeCachedIR(path string, cached *cachedIR) {
	if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil {
		fmt.Fprintf(os.Stderr, "decodo: failed to create cache directory: %v\n", err)
		return
	}
	data, err := json.MarshalIndent(cached, "", "  ")
	if err != nil {
		fmt.Fprintf(os.Stderr, "decodo: failed to marshal cache: %v\n", err)
		return
	}
	if err := os.WriteFile(path, data, 0644); err != nil {
		fmt.Fprintf(os.Stderr, "decodo: failed to write cache: %v\n", err)
	}
}
...
func buildRemoteSchema(ir *remoteIR) (*RemoteSchema, error) {
	...
	for key, target := range ir.APIs.WebScrapingAPI.Targets {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "decodo: failed to marshal schema for target %q: %v\n", key, err)
			continue
		}
		schemaURL := fmt.Sprintf("mem://%s.json", key)
		if err := compiler.AddResource(schemaURL, strings.NewReader(string(schemaJSON))); err != nil {
			fmt.Fprintf(os.Stderr, "decodo: failed to add schema resource for target %q: %v\n", key, err)
			continue
		}
		compiled, err := compiler.Compile(schemaURL)
		if err != nil {
			fmt.Fprintf(os.Stderr, "decodo: failed to compile schema for target %q: %v\n", key, err)
			continue
		}
...
func (s *RemoteSchema) GetRequestSchema(target string) *jsonschema.Schema {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *RemoteSchema) ListTargets() []string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	out := make([]string, len(s.targetKeys))
	copy(out, s.targetKeys)
	sort.Strings(out)
	return out
}
